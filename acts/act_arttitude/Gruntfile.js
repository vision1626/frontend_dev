...
module.exports = function (grunt) {
    ...
    var globalConfig = {
        ...
        dest: 'dev',
        actName:'arttitude',
        actDName:'arttitude-d',
        cssSrcPath: 'source/less',
        jsSrcPath: 'source/coffee',
        jadeSrcPath: 'source/jade',
        cssDistPath: '../../tpl/hi1626/css/act/<%= globalConfig.actName %>',
        jsDebugPath: '../../tpl/hi1626/js/act/<%= globalConfig.actName %>/debug',
        jsDistPath: '../../tpl/hi1626/js/act/<%= globalConfig.actName %>',
        htmlDistPath: '../../tpl/hi1626/page/act/<%= globalConfig.actName %>',
        cssName: '<%= globalConfig.actName %>',
        cssDName: '<%= globalConfig.actDName %>',
        jsName: '<%= globalConfig.actName %>',
        jsDName: '<%= globalConfig.actDName %>'
    };
        ...
        cssmin: {
            target: {
                files: {
                    ...
                    '<%= globalConfig.cssDistPath %>/<%= globalConfig.cssDName %>.min.css': [
                        '<%= globalConfig.cssDistPath %>/<%= globalConfig.cssDName %>.min.css'
                    ]
                }
            }
        ...
        less: {
            dist: {
                files: {
                    ...
                    '<%= globalConfig.cssDistPath %>/<%= globalConfig.cssDName %>.min.css': [
                        '<%= globalConfig.cssSrcPath %>/app--d.less'
                    ]
        ...
        coffee: {
            ...
            compile: {
                files: {
                    ...
                    '<%= globalConfig.jsDebugPath %>/<%= globalConfig.jsDName %>.js':[
                        '<%= globalConfig.jsSrcPath %>/util.coffee',
                        '<%= globalConfig.jsSrcPath %>/arttitude_d.coffee'
                    ]
                }
            }
        },
